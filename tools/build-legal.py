...
PRIVACY = [
    ...
    ("p", "Our WordPress enquiry forms send your details to our configured enquiry service by email, and also to BizzFlowUK, the customer management system we use to record, manage and respond to enquiries. BizzFlowUK is provided to us by LaunchFlow UK Limited, which processes this information on our behalf. [CONFIRM A DATA PROCESSING AGREEMENT WITH LAUNCHFLOW UK LIMITED IS IN PLACE.] Access is restricted to authorised people who need it. We use [IDENTIFY HOSTING AND MAIL PROVIDERS OR CLEAR RECIPIENT CATEGORIES] to operate the website and handle enquiries. These providers process information for the purposes described here under the relevant arrangements."),
    ("p", "If BizzFlowUK cannot be reached at the moment you submit an enquiry, the website keeps a copy for no longer than 24 hours while it retries, then deletes it whether or not delivery succeeded. [STATE WHETHER ENQUIRY STORAGE IN WORDPRESS IS SWITCHED ON; IF IT IS, DESCRIBE IT HERE AND GIVE ITS RETENTION PERIOD.]"),
    ("h", "Transfers outside the UK"),
    ("p", "[STATE ACTUAL PROCESSING LOCATIONS, INCLUDING WHERE BIZZFLOWUK HOSTS ITS DATA - CONFIRM THE REGION WITH LAUNCHFLOW UK LIMITED. IF RELEVANT, EXPLAIN THE APPLICABLE TRANSFER ARRANGEMENT AND HOW TO OBTAIN INFORMATION ABOUT SAFEGUARDS. DO NOT ASSERT UK-ONLY STORAGE WITHOUT EVIDENCE.]"),
